...
function makeSmokeBadge(label, message, color) {
  const lw = label.length > 8 ? 72 : 62;
  const mw = message.length * 7 + 14;
  const tw = lw + mw;
  return '<svg xmlns="http://www.w3.org/2000/svg" width="' + tw + '" height="20" role="img"><title>' + label + ': ' + message + '</title><linearGradient id="s" x2="0" y2="100%"><stop offset="0" stop-color="#bbb" stop-opacity=".1"/><stop offset="1" stop-opacity=".1"/></linearGradient><clipPath id="r"><rect width="' + tw + '" height="20" rx="3" fill="#fff"/></clipPath><g clip-path="url(#r)"><rect width="' + lw + '" height="20" fill="#555"/><rect x="' + lw + '" width="' + mw + '" height="20" fill="' + color + '"/><rect width="' + tw + '" height="20" fill="url(#s)"/></g><g fill="#fff" text-anchor="middle" font-family="DejaVu Sans,Verdana,Geneva,sans-serif" font-size="11"><text x="' + Math.round(lw/2) + '" y="14">' + label + '</text><text x="' + Math.round(lw+mw/2) + '" y="14">' + message + '</text></g></svg>';
}
...
<body>
  ...
  if (url.pathname === "/token" && req.method === "POST") {
    ...
    if (!verifyPKCE(codeVerifier, stored.codeChallenge)) {
        // ── Smoke test badge (SVG, links to latest run via smoke-status.json) ────────
  if (url.pathname === '/badge/smoke' && req.method === 'GET') {
    try {
      ...
      const svg = makeSmokeBadge('smoke test', message, color);
      ...
      res.end(makeSmokeBadge('smoke test', 'unknown', '#9f9f9f'));
    }
    return;
  }

  // ── Smoke test redirect — links badge click to latest run summary ─────────
  ...
  if (url.pathname === '/badge/coolify' && req.method === 'GET') {
    try {
      const appRes = await coolifyFetch('/applications/tuk1rcjj16vlk33jrbx3c9d3');
      const app = typeof appRes === 'string' ? JSON.parse(appRes) : appRes;
      ...
      const svg = makeSmokeBadge('coolify', message, color);
      res.writeHead(200, { 'Content-Type': 'image/svg+xml', 'Cache-Control': 'no-cache, no-store, must-revalidate', 'Pragma': 'no-cache', 'Expires': '0' });
      res.end(svg);
    } catch (err) {
      res.writeHead(200, { 'Content-Type': 'image/svg+xml', 'Cache-Control': 'no-cache' });
      res.end(makeSmokeBadge('coolify', 'unknown', '#9f9f9f'));
    }
    return;
  }
