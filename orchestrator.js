...
async function runManager() {
  ...
  const system = [
    ...
    "",
    "TOOLS NOT AVAILABLE:",
    "  - read_file: NOT available. CLAUDE_TEAM.md, TASK_BOARD, BUILD_LOG, QA_REPORT,",
    "    OPERATOR_INBOX, OBSERVER_INBOX are ALL already injected in this message.",
    "  - coolify_list_deployments / coolify_deployment_logs: NOT available.",
    "    Deployment state is in LIVE DATA already in your context.",
    "",
    "CAPS:",
    "  - run_command output capped at 5000 chars. Use grep/head for targeted verification.",
    "  - Example to verify app health: run_command 'curl -s https://cuttingedgechat.com/api/version'",
    "  - Example to verify script exists: run_command 'ls /repo-observer/scripts/'",
...
async function runOperator() {
  ...
  const system = [
    ...
    "",
    "TOOLS NOT AVAILABLE — do NOT attempt to call these:",
    "  - read_file: NOT available. The orchestrator pre-fetches all file contents and",
    "    injects them as text in your context (CLAUDE_TEAM.md, TASK_BOARD, BUILD_LOG,",
    "    OPERATOR_INBOX, CODEBASE_REFERENCE are all already in this message).",
    "  - list_directory: NOT available. Use run_command with ls if needed.",
    "  - coolify_list_deployments / coolify_deployment_logs / coolify_list_envs: NOT available.",
    "    Coolify deployment state is in LIVE DATA already injected in your context.",
    "",
    "IMPORTANT CAPS — plan your commands accordingly:",
    "  - run_command output is HARD CAPPED at 5000 chars. If a command produces more,",
    "    you will see '[...output truncated...]' at the end. Use grep/head/tail to",
    "    target specific output rather than running commands that dump everything.",
    "  - Example: instead of 'cat src/libs/auth-provider/index.ts' (may truncate),",
    "    use 'head -50 src/libs/auth-provider/index.ts' or grep for specific patterns.",
    "  - write_file has no cap — write full file content as needed.",
    "",
...
async function runObserver() {
  ...
  const system = [
    ...
    "",
    "TOOLS NOT AVAILABLE:",
    "  - read_file: NOT available. Context files are pre-fetched and already in this message.",
    "    If you need to inspect a file, use: run_command with 'cat', 'head', or 'grep'.",
    "",
    "CAPS:",
    "  - run_command output capped at 5000 chars. Use head/grep/tail for targeted output.",
    "  - T-001 script exits with code 1 when tests fail — that is NORMAL, not an error.",
    "    The orchestrator captures stdout even on non-zero exit. You will see the results.",
